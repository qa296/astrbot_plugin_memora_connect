...
class MemoryAPIGateway:
    ...
    @performance_monitored
    async def batch_get_intimacy(self, user_ids: List[str], group_id: str = "") -> APIResponse:
        """
        批量获取用户亲密度
    ...
    @performance_monitored
    async def find_connection(self, user_a: str, user_b: str, group_id: str = "") -> APIResponse:
        """
        查找两个用户的关系路径（共同兴趣话题）
        ...
        try:
            # 获取两个用户的兴趣
            interests_a = await self.user_profiling.extract_user_interests(user_a, group_id, top_k=10)
            interests_b = await self.user_profiling.extract_user_interests(user_b, group_id, top_k=10)
            ...
            return APIResponse(
                ...
                data={
                    ...
                    "user_a_interests": [{"concept": c, "weight": w} for c, w in interests_a[:5]],
                    "user_b_interests": [{"concept": c, "weight": w} for c, w in interests_b[:5]]
                },
                latency_ms=(time.time() - start_time) * 1000
            )
    ...
    @performance_monitored
    async def get_memory_importance_ranking(self, group_id: str = "", top_k: int = 10) -> APIResponse:
        """
        获取记忆重要性排序
        基于度中心性和激活频率

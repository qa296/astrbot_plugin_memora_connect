...
class EnhancedMemoryRecall:
    ...
    async def _semantic_recall(
        ...
        try:
            ...
            logger.debug(
                f"开始语义召回，查询: {query}, 记忆总数: {len(memories_snapshot)}"
            )
            ...
            for memory in memories_snapshot:
                ...
                task = asyncio.create_task(
                    self.memory_system.embedding_cache.get_embedding(
                        memory.id, memory.content, group_id
                    )
                )
            ...
            for memory in memories_snapshot:
                if memory.id in memory_embeddings:
                    similarity = self._cosine_similarity(
                        query_embedding, memory_embeddings[memory.id]
                    )
    ...
    async def _strength_based_recall(
        self, query: str, group_id: str = ""
    ) -> list[MemoryRecallResult]:
        """基于记忆强度的召回"""
        try:
    ...
    def _deduplicate_and_rank(
        self, results: list[MemoryRecallResult]
    ) -> list[MemoryRecallResult]:
        """去重并按相关性排序"""
        try:
            # 按记忆内容去重
        ...
        try:
            ...
            if "keyword" in grouped_memories:
                ...
                if keyword_memories:
                    concepts = list(
                        set(
                            [
                                m.metadata.get("concept_name", "")
                                for m in keyword_memories
                            ]
                        )
                    )
                    summary_parts.append(
                        f"关键词匹配: 涉及{', '.join(concepts[:2])}等主题"
                    )
            ...
            if "associative" in grouped_memories:
                ...
                if associative_memories:
                    summary_parts.append(
                        f"联想记忆: {len(associative_memories)}条相关记忆"
                    )
    ...
    def format_memories_for_llm(
        self, memories: list[MemoryRecallResult], include_ids: bool = False
    ) -> str:
        """将记忆格式化为LLM友好的上下文"""
        try:
            if not memories:
            ...
            for i, memory in enumerate(memories[:5], 1):  # 最多5条
                ...
                if include_ids:
                    memory_id = (
                        memory.metadata.get("memory_id") if memory.metadata else None
                    )
    ...
    async def _keyword_recall(
        self, query: str, group_id: str = "", keywords: list[str] = None
    ) -> list[MemoryRecallResult]:
        """基于关键词的召回"""
        try:
            if not query:
    ...
    def _filter_injection_results(
        ...
        try:
            ...
            semantic_primary_concepts = {
                r.concept_id
                for r in primary_results
                if r.memory_type == "semantic" and r.concept_id
            }
            ...
            for result in results:
                ...
                if result.memory_type == "keyword":
                    ...
                    if matched >= min_keyword_matches and ratio >= 0.5:
                        if (
                            not semantic_primary_effective
                            or result.concept_id in semantic_primary_concepts
                        ):
                            filtered.append(result)
    ...
    async def recall_relevant_memories_for_injection(
        self, message: str, group_id: str = ""
    ) -> list[MemoryRecallResult]:
        """为自动注入召回相关记忆"""
        try:
            if not self.memory_system.memory_graph.memories:
            ...
            enable_associative = self.memory_system.memory_config.get(
            ...
            keyword_results = await self._keyword_recall(
                message, group_id, keywords=keywords
            )
            ...
            # 注入专用过滤
            filtered_results = self._filter_injection_results(
                unique_results, keywords, semantic_primary
            )

            # 限制数量
            max_memories = self.memory_system.memory_config.get(
    ...
    def should_inject_memories(self, memories: list[MemoryRecallResult]) -> bool:
        ...
        threshold = self.memory_system.memory_config.get(
            "memory_injection_threshold", 0.3
        )
    ...
    async def trigger_precomputation_for_uncached_memories(
        self, memory_ids: list[str] = None
    ) -> bool:
        """触发未缓存认忆的预计算任务"""
        try:
            if not self.memory_system.embedding_cache:
            ...
            for memory_id in memory_ids:
                ...
                if memory:
                    memory_group_id = getattr(memory, "group_id", "")
                    if not await self.memory_system.embedding_cache._get_cached_embedding(
                        memory_id, memory_group_id
                    ):

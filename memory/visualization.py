...
class MemoryGraphVisualizer:
    ...
    def __init__(self, memory_system: Any) -> None:
        ...
        try:
            ...
            font_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "font.otf"
            )
    ...
    async def generate_graph_image(
        ...
        try:
            ...
            # 在主事件循环中获取所有需要的数据
            graph_data = await self._prepare_graph_data(
                max_nodes, max_edges, edge_strength_threshold, actual_group_id
            )

            # 将数据传递给同步绘图函数，在线程池中只执行纯绘图操作
            return await asyncio.to_thread(
                self._generate_graph_image_sync, graph_data, layout_style
            )
        except Exception as e:
            logger.error(f"生成记忆图谱失败: {e}", exc_info=True)
            return ""

    async def _prepare_graph_data(
        self,
        max_nodes: int,
        max_edges: int,
    ...
    def _generate_graph_image_sync(
        self, graph_data: dict[str, Any], layout_style: str
    ) -> str:
        if nx is None:
        ...
        for node in nodes_data:
            G.add_node(
                ...
                name=node["name"],
                count=node["count"],
                avg_strength=node["avg_strength"],
                max_strength=node["max_strength"],
            )
        ...
        if layout_style == "auto":
            ...
            if num_nodes <= 5:
                ...
                layout_style = (
                    "community" if community_louvain is not None else "force_directed"
                )
            else:
                # 大量节点，使用多层次布局
        ...
        if G.number_of_nodes() > 25 and G.number_of_edges() > 30:
            ...
            try:
                ...
                # 保留前70%的边
                keep_edges = [
                    e[0] for e in sorted_edges[: int(len(sorted_edges) * 0.7)]
                ]
                # 移除不重要的边
        ...
        if layout_style == "force_directed":
            ...
            for node in G.nodes():
                if node in pos_layout:
                    ...
                    if neighbors:
                        # 计算邻居节点的平均位置
                        avg_x = sum(
                            pos_layout.get(n, (0, 0))[0]
                            for n in neighbors
                            if n in pos_layout
                        ) / len(neighbors)
                        avg_y = sum(
                            pos_layout.get(n, (0, 0))[1]
                            for n in neighbors
                            if n in pos_layout
                        ) / len(neighbors)
                        # 添加一些随机偏移，避免重叠
            ...
            for node in G.nodes():
                ...
                importance = (
                    0.4 * (count / max_count)
                    + 0.3 * (avg_strength / max_avg_strength)
                    + 0.3 * (max_strength / max_max_strength)
                )
            ...
            for layer_name, nodes in layers.items():
                ...
                if subgraph.number_of_nodes() == 1:
                    ...
                    layer_positions[layer_name] = {
                        nodes[0]: (0, layer_y_positions[layer_name])
                    }
                else:
                    # 多节点使用力导向布局
                    # 根据节点数量调整布局参数
                    ...
                    for node, (x, y) in sub_pos.items():
                        layer_positions[layer_name][node] = (
                            x,
                            layer_y_positions[layer_name] + y * 0.3,
                        )
            ...
            if community_louvain is None:
                logger.warning(
                    "未安装 python-louvain，无法使用社区布局，降级为 force_directed"
                )
                ...
                for comm_id, nodes_in_comm in communities.items():
                    ...
                    if subgraph.number_of_nodes() > 1:
                        # 社区内部也使用多阶段布局
                        sub_initial_pos = nx.circular_layout(
                            subgraph, scale=intra_community_scale
                        )
                        sub_pos = nx.spring_layout(
                            subgraph,
                            pos=sub_initial_pos,
                ...
                for u, v, data in G.edges(data=True):
                    ...
                    if comm_u != comm_v:
                        ...
                        inter_comm_weights[key] = inter_comm_weights.get(
                            key, 0
                ...
                if community_graph.number_of_nodes() > 1:
                    # 社区间布局也使用多阶段布局
                    comm_initial_pos = nx.circular_layout(
                        community_graph, scale=5.0
                    )  # 社区间距离更大
                    inter_community_pos = nx.spring_layout(
                        community_graph,
                        pos=comm_initial_pos,
                        weight="weight",
                        k=2.0
                        / math.sqrt(community_graph.number_of_nodes()),  # 社区间k值较大
                        iterations=1000,
                        seed=42,
                        scale=5.0,
                    )
        ...
        ax.set_axis_off()
        ax.set_title(
            f"记忆图谱  节点:{G.number_of_nodes()} 边:{G.number_of_edges()}",
            fontsize=12,
            pad=12,
        ...
        # 11) 保存到数据目录 graphs/
        data_dir = (
            os.path.dirname(self.ms.db_path)
            if hasattr(self.ms, "db_path")
            else os.getcwd()
        )
    ...
    def _apply_collision_detection(
        self,
        G: Any,
        pos: dict[str, tuple],
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(nodes)):
                for j in range(i + 1, len(nodes)):
                    ...
                    min_distance = (
                        node_radii[node_i] + node_radii[node_j] + buffer_factor
                    )
                    ...
                    if distance < min_distance and distance > 0:
                        ...
                        move_i = (
                            overlap
                            * force_multiplier
                            * (node_radii[node_j] / total_size)
                        )
                        move_j = (
                            overlap
                            * force_multiplier
                            * (node_radii[node_i] / total_size)
                        )

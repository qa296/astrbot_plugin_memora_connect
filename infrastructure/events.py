...
class MemoryEventBus:
    ...
    def subscribe(self, event_type: MemoryEventType, callback: Callable) -> bool:
        ...
        try:
            ...
            if callback not in self._subscribers[event_type]:
                ...
                logger.info(
                    f"订阅事件成功: {event_type.value}, 回调: {callback.__name__}"
                )
                return True
            else:
                logger.warning(
                    f"重复订阅事件: {event_type.value}, 回调: {callback.__name__}"
                )
                return False
        ...
        try:
            if event_type in self._subscribers:
                if callback in self._subscribers[event_type]:
                    ...
                    logger.info(
                        f"取消订阅成功: {event_type.value}, 回调: {callback.__name__}"
                    )
                    return True
        ...
        try:
            ...
            if async_mode:
                ...
                try:
                    ...
                    logger.warning(
                        f"事件队列已满，事件被丢弃: {event.event_type.value}"
                    )
                    return False
            else:
                # 同步模式：立即处理
    ...
    async def _process_event(self, event: MemoryEvent):
        ...
        logger.debug(
            f"处理事件: {event.event_type.value}, 订阅者数量: {len(subscribers)}"
        )
        ...
        if tasks:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    logger.error(
                        f"事件回调执行失败: {subscribers[i].__name__}, 错误: {result}"
                    )
    ...
    def get_event_history(
        self, event_type: MemoryEventType | None = None, limit: int = 100
    ) -> list[MemoryEvent]:
        """
        获取事件历史
...
# 全局事件总线实例（单例模式）
_global_event_bus: MemoryEventBus | None = None

...
class UserProfilingSystem:
    ...
    async def calculate_intimacy(self, user_id: str, group_id: str = "", 
                                force_recalculate: bool = False) -> IntimacyScore:
        """
        计算用户亲密度
        ...
        try:
            ...
            # 1. 计算互动频度
            user_memories = [
                m for m in memory_graph.memories.values()
            ...
            if user_memories:
                ...
                score.interaction_frequency = min(1.0, score.total_interactions / (days_known * 5))
                ...
                # 3. 计算情感价值
                # 基于印象系统的好感度
                impression = self.memory_system.get_person_impression_summary(group_id, user_id)
    ...
    async def batch_get_intimacy(self, user_ids: List[str], group_id: str = "") -> List[Dict]:
        """
        批量获取用户亲密度
    ...
    async def extract_user_interests(self, user_id: str, group_id: str = "", 
        ...
        try:
            ...
            if cache_key in self._interest_cache:
                ...
                sorted_interests = sorted(cached_interests, key=lambda x: x.weight, reverse=True)
    ...
    async def _save_interests_to_db(self, user_id: str, group_id: str, interests: List[UserInterest]):
        """保存兴趣到数据库"""
        try:
            db_path = self.memory_system.db_path
            conn = sqlite3.connect(db_path)
            ...
            # 先删除旧记录
            cursor.execute("DELETE FROM user_interests WHERE user_id = ? AND group_id = ?", 
                          (user_id, group_id))
    ...
    async def get_user_interests(self, user_id: str, group_id: str = "") -> List[Dict]:
        ...
        return [
            {"concept": name, "weight": weight}
            for name, weight in interests
        ]
    
    async def add_taboo_word(self, user_id: str, word: str, reason: str = "", group_id: str = ""):
        """
        添加禁忌词
    ...
    async def check_taboo(self, user_id: str, content: str, group_id: str = "") -> List[str]:
        """
        检查内容是否包含禁忌词
    ...
    async def learn_taboo_from_message(self, user_id: str, message: str, group_id: str = ""):
        """
        从消息中自动学习禁忌词
        检测用户的反感表达
